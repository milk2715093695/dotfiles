# render_config_file — 按需处理 # @dotfiles:<file> 标记，将基础配置与 locals 拼接输出
#
# 本文件提供通用工具函数，由各 render_xxx_config() 按需调用。
# 当基础配置中尚未添加 @dotfiles: 标记时，render 函数不需要调用本工具，
...
render_config_file() {
    ...
    local marker_regex="^[[:space:]]*${comment_char}[[:space:]]*@dotfiles:(.+)[[:space:]]*$"
    ...
    # 快路径：无标记时直接复制
    if ! grep -q "^[[:space:]]*${comment_char}[[:space:]]*@dotfiles:" "$source" 2>/dev/null; then
        cp "$source" "$output"
        return 0
    fi
    ...
    if [ -d "$locals_dir" ]; then
        for local_file in "$locals_dir"/*; do
            ...
            case "$referenced" in
                *" $base "*) ;;
                *) warn "locals 文件未被任何 @dotfiles: 标记引用：${locals_dir}/${base}" ;;
            esac
        done
    fi
}
